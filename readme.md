...
| `GOOGLE_MAPS_MAP_ID`      | A Google Maps Map ID (vector map). Required by `AdvancedMarkerElement` and the modern Maps JS rendering pipeline. Defaults to Google's `DEMO_MAP_ID` for local development; production should use a Map ID created in Google Cloud. | `DEMO_MAP_ID` | No       |
